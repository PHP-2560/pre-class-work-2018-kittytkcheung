...
```{r}
mcGamblers_auto = function(bankroll = 50000, betCost = 100, numHands = 100, prob1 = .46, prob2 = .5) {
  iter = c(1:100)
  init = (prob1 + prob2)/2
  difference = -999999
  while (difference <= -10000 |difference >= 10000) {
    for (j in iter) {
      hands = c(1:numHands)
      ...
      difference = bankroll - 50000
  
      if (difference > 0) {
        init = (prob1 + (prob1 + prob2)/2)/2
      } else {
        init = ((prob1 + prob2)/2 + prob2)/2
      }
    }
  }
  return(c(difference, bankroll,init,i))
}
```
